...
prog_B_est=function(u,y){
        ...
        list(B.prog=B.prog,invB.B=invB.B) 
}
...
phi_est=function(y, B.prog,invB.B, muhat){
        N=length(y[,1])
        M=length(y[1,])
      ...
      B.new=mat.or.vec(M1^2,(Ns2+degree+1)^2)
       ...
       xlab = "M1", ylab = "M1", zlab = "Ghat.new")
        ...
        list(lamdahat=lamdahat,phihat=phihat)
...
beta_est=function(y,muhat,phihat,lamdahat){
       ...
       u1=c(0,u)
          ...
          if(kapa==1){
               ...
               for (i in 2:(M+1)){
                 temp1= (u1[i]-u1[i-1])*(y-temp)%*%phihat
                 betahat=betahat+temp1
                }
           }
      ...
      if(kapa>1){
          for(j in 1:kapa){
          for (i in 2:(M+1))
              temp2=(u1[i]-u1[i-1])*(y-temp)%*%phihat[,j]
              betahat[,j]=betahat[,j]+temp2
          }
          }
          
        list(betahat=betahat)
        #plot(beta1,type="b")
        #points(betahat[,1],type="b",col=2)
}
...
x_predict=function(h = 1, nroll = 30, nhis = N, first = N+1, y, x, u){
        ...
        for (i in 1:nroll){
            ...
            phi.est=phi_est(data.hist,B.prog,muhat)
            ...
            plot(x.pred[i,],type="l",ylab="x",main=paste(as.character(i),"step ahead prediction"))
            points(x[(nhis+i),],col=2)
